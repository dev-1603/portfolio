<script lang="ts">
  ...
  let filteredProjects = professionalProjects?.sort((a, b) => (a.order || 0) - (b.order || 0));
  ...
  function switchTab(tab: 'professional' | 'personal' | 'github') {
    ...
    switch (tab) {
      case 'professional':
        filteredProjects = professionalProjects.sort((a, b) => (a.order || 0) - (b.order || 0));
        break;
      case 'personal':
        filteredProjects = personalProjects.sort((a, b) => (a.order || 0) - (b.order || 0));
        break;
      case 'github':
        filteredProjects = [];
        break;
  ...
  function handleNpmModal(event: CustomEvent) {
    openNpmModal(event.detail.project);
  }
</script>
